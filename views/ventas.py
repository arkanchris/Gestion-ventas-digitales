...
class VentasView(ctk.CTkFrame):
    ...
    def _build_form(self):
        ...
        self.prov_tel    = {p["nombre"]: p.get("telefono", "") for p in proveedores}
        self.e_proveedor = _combo(r2, prov_names)
        self.e_proveedor.grid(row=0, column=3, sticky="ew", padx=(8, 0))
        # ▶ Auto-fill teléfono al cambiar distribuidor
        self.e_proveedor.configure(command=self._on_distribuidor_change)
    ...
    # ═══════════════════════════════════════════════════════════
    #  AUTO-FILL DISTRIBUIDOR → TELÉFONO
    # ═══════════════════════════════════════════════════════════
    def _on_distribuidor_change(self, nombre):
        tel = self.prov_tel.get(nombre, "")
        self.e_telefono.delete(0, "end")
        if tel:
            self.e_telefono.insert(0, tel)
    ...
    def _build_single_area(self, s):
        ...
        # ── DATOS DE ACCESO — layout compacto ──
        section_header(s, "🔐  Datos de Acceso")
        af = card(s)
        af.pack(fill="x", padx=24, pady=4)

        # Fila A: Correo (más ancho) | Contraseña | PIN (corto) | Perfil (corto)
        ra = ctk.CTkFrame(af, fg_color="transparent")
        ra.pack(fill="x", padx=16, pady=10)
        # columnas: label(0) correo(1) | label(2) pass(3) | label(4) pin(5) | label(6) perfil(7)
        ra.grid_columnconfigure(1, weight=3)   # correo — más ancho
        ra.grid_columnconfigure(3, weight=3)   # contraseña
        ra.grid_columnconfigure(5, weight=1)   # pin — corto
        ra.grid_columnconfigure(7, weight=1)   # perfil — corto

        _lbl2(ra, "Correo/Usuario", 0)
        self.e_correo = entry_field(ra, placeholder="correo@ejemplo.com")
        self.e_correo.grid(row=0, column=1, sticky="ew", padx=(6, 14))

        _lbl2(ra, "Contraseña", 2)
        self.e_password = entry_field(ra, placeholder="Contraseña")
        self.e_password.grid(row=0, column=3, sticky="ew", padx=(6, 14))

        _lbl2(ra, "PIN", 4)
        self.e_pin = entry_field(ra, placeholder="PIN")
        self.e_pin.grid(row=0, column=5, sticky="ew", padx=(6, 14))

        _lbl2(ra, "Perfil", 6)
        self.e_perfil = entry_field(ra, placeholder="Perfil")
        self.e_perfil.grid(row=0, column=7, sticky="ew", padx=(6, 0))
        ...
        for c in (1, 3, 5):
            r5.grid_columnconfigure(c, weight=1)
        ...
        # Fecha activación = hoy por defecto
        ...
        if plataformas and not self.editing_id:
            ...
            p0 = self.plat_precio.get(first, 0)
            if p0:
                self.e_precio.delete(0, "end")
                self.e_precio.insert(0, str(int(p0)))
        ...
        primary_btn(bf, "💾  Guardar Venta",
                    command=self._guardar_simple).pack(side="left", padx=(0, 8))
    ...
    def _build_multiple_area(self, s):
        ...
        # ── Datos de acceso compactos ──
        ra = ctk.CTkFrame(pf, fg_color="transparent")
        ra.pack(fill="x", padx=16, pady=8)
        ra.grid_columnconfigure(1, weight=3)
        ra.grid_columnconfigure(3, weight=3)
        ra.grid_columnconfigure(5, weight=1)
        ra.grid_columnconfigure(7, weight=1)

        _lbl2(ra, "Correo/Usuario", 0)
        self.m_correo = entry_field(ra, placeholder="correo@ejemplo.com")
        self.m_correo.grid(row=0, column=1, sticky="ew", padx=(6, 14))

        _lbl2(ra, "Contraseña", 2)
        self.m_password = entry_field(ra, placeholder="Contraseña")
        self.m_password.grid(row=0, column=3, sticky="ew", padx=(6, 14))

        _lbl2(ra, "PIN", 4)
        self.m_pin = entry_field(ra, placeholder="PIN")
        self.m_pin.grid(row=0, column=5, sticky="ew", padx=(6, 14))

        _lbl2(ra, "Perfil", 6)
        self.m_perfil = entry_field(ra, placeholder="Perfil")
        self.m_perfil.grid(row=0, column=7, sticky="ew", padx=(6, 0))

        r5 = ctk.CTkFrame(pf, fg_color="transparent")
        r5.pack(fill="x", padx=16, pady=8)
        for c in (1, 3, 5):
            r5.grid_columnconfigure(c, weight=1)
        _lbl(r5, "Precio *", 0)
        ...
        r5b = ctk.CTkFrame(pf, fg_color="transparent")
        r5b.pack(fill="x", padx=16, pady=(0, 8))
        ...
        self.m_estado.pack(side="left", padx=10)
        ...
        # Precargar si está editando
        if self.editing_item is not None:
            item = self.items_multiple[self.editing_item]
            self.m_plataforma.set(item.get("plat_nombre", ""))
            _set_e(self.m_correo,   item.get("correo_usuario", ""))
            _set_e(self.m_password, item.get("contrasena", ""))
            _set_e(self.m_pin,      item.get("pin", ""))
            _set_e(self.m_perfil,   item.get("perfil", ""))
            _set_e(self.m_precio,   str(item.get("precio_venta", "")))
            ...
            _set_e(self.m_notas,    item.get("notas", ""))
        else:
            # Fecha activación = hoy + precio por defecto
            from datetime import date
            self.m_fecha_act.set(date.today().strftime("%Y-%m-%d"))
            if plataformas:
                p0 = self.plat_precio_m.get(plataformas[0]["nombre"], 0)
                if p0:
                    self.m_precio.delete(0, "end")
                    self.m_precio.insert(0, str(int(p0)))

        # Botón agregar / guardar edición
        add_f = ctk.CTkFrame(pf, fg_color="transparent")
        add_f.pack(fill="x", padx=16, pady=(0, 12))
        if self.editing_item is not None:
            ctk.CTkButton(add_f, text="💾  Guardar cambios",
                          command=self._guardar_edicion_item,
                          height=38, corner_radius=8,
                          fg_color="#0e6027", hover_color="#0b4d1f",
                          font=ctk.CTkFont(size=13, weight="bold"),
                          text_color="white").pack(side="left", padx=(0, 8))
            secondary_btn(add_f, "✕  Cancelar edición",
    ...
    # ═══════════════════════════════════════════════════════════
    #  LISTA DE ITEMS — centrada y alineada
    # ═══════════════════════════════════════════════════════════
    def _refresh_items_list(self):
        ...
        if not self.items_multiple:
            ctk.CTkLabel(self.items_frame,
                         text="Aún no agregaste ninguna plataforma. Rellena el formulario arriba.",
        ...
        for col, (txt, w) in enumerate(zip(HEADERS, COL_W)):
            ctk.CTkLabel(hdr, text=txt, width=w, anchor="center",
                         ...
                         ).grid(row=0, column=col, padx=3, pady=6, sticky="ew")
    ...
    def _on_plataforma_change(self, nombre):
        if hasattr(self, "plat_precio") and hasattr(self, "e_precio"):
            p = self.plat_precio.get(nombre, 0)
            if p:
                self.e_precio.delete(0, "end")
                self.e_precio.insert(0, str(int(p)))

    def _on_m_plataforma_change(self, nombre):
        if hasattr(self, "plat_precio_m") and hasattr(self, "m_precio"):
            p = self.plat_precio_m.get(nombre, 0)
            if p:
                self.m_precio.delete(0, "end")
                self.m_precio.insert(0, str(int(p)))

    # ═══════════════════════════════════════════════════════════
    #  LÓGICA DE ITEMS MÚLTIPLES
    # ═══════════════════════════════════════════════════════════
    def _agregar_item(self):
        if not self.e_cliente.get().strip():
            messagebox.showerror("Error", "Primero escribe el nombre del cliente."); return
        ...
        if precio <= 0:
            messagebox.showerror("Error", "El precio debe ser mayor a 0."); return
        ...
        for e in [self.m_correo, self.m_password, self.m_pin,
            ...
            e.delete(0, "end")
        self.m_fecha_act.set("")
        self.m_fecha_venc.set("")
        self.m_estado.set("pagada")
        self._refresh_items_list()
    ...
    def _guardar_edicion_item(self):
        ...
        self.items_multiple[self.editing_item] = {
            "plataforma_id": plat_id, "plat_nombre": plat_nombre,
            "correo_usuario":    self.m_correo.get().strip(),
            "contrasena":        self.m_password.get().strip(),
            "pin":               self.m_pin.get().strip(),
            "perfil":            self.m_perfil.get().strip(),
    ...
    def _guardar_multiple(self):
        ...
        for item in self.items_multiple:
            data = {
                "cliente": cliente, "perfil": item.get("perfil",""),
                "telefono": telefono, "plataforma_id": item["plataforma_id"],
                "orden_compra": orden, "correo_usuario": item.get("correo_usuario",""),
                "proveedor_id": prov_id, "contrasena": item.get("contrasena",""),
                "pin": item.get("pin",""), "precio_venta": item.get("precio_venta",0),
        ...
        if resp:
            ...
            generar_tirilla_multiple(
                ventas=ventas_g, cliente=cliente, telefono=telefono,
                config=config, notas_generales=notas_g,
    ...
    def _get_single_data(self):
        ...
        return {
            ...
            "proveedor_id":      self.prov_map.get(self.e_proveedor.get()),
...
def _lbl(parent, text, col, width=115):
    ctk.CTkLabel(parent, text=text, anchor="w", width=width,
                 font=ctk.CTkFont(size=13),
                 text_color=COLORS["text_dim"]).grid(row=0, column=col, sticky="w")

def _lbl2(parent, text, col, width=90):
    """Label más compacto para la fila de datos de acceso."""
    ctk.CTkLabel(parent, text=text, anchor="w", width=width,
                 font=ctk.CTkFont(size=12),
                 text_color=COLORS["text_dim"]).grid(row=0, column=col, sticky="w", padx=(4,0))
