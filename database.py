...
class Database:
    ...
    def init_db(self):
        ...
        # Migration: add imagen_path if it doesn't exist
        try:
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS proveedores (
            ...
            telefono TEXT,
            correo TEXT,
            maneja_credito INTEGER DEFAULT 0,
            notas TEXT,
    ...
    def get_config(self):
        conn = self.get_conn()
        rows = conn.execute(
            "SELECT clave, valor FROM configuracion").fetchall()
        conn.close()
    ...
    def get_plataformas(self, solo_activas=False):
        ...
        if solo_activas:
            rows = conn.execute(
                "SELECT * FROM plataformas WHERE activa=1 ORDER BY nombre"
            ).fetchall()
        else:
            rows = conn.execute(
    ...
    def get_ventas(self, filtros=None):
        conn = self.get_conn()
        query = """SELECT v.*, p.nombre as plataforma_nombre,
        ...
        if filtros:
            ...
            if filtros.get("busqueda"):
                ...
                query += " AND (v.cliente LIKE ? OR v.telefono LIKE ? OR v.correo_usuario LIKE ?)"
                params.extend([b, b, b])
            if filtros.get("fecha_desde"):
                query += " AND v.fecha_activacion >= ?"
    ...
    def update_venta(self, vid, data):
        conn = self.get_conn()
        conn.execute("""UPDATE ventas SET cliente=?, perfil=?, telefono=?,
                        plataforma_id=?, orden_compra=?, correo_usuario=?,
                        proveedor_id=?, contrasena=?, pin=?, precio_venta=?,
                        fecha_activacion=?, fecha_vencimiento=?, notas=?,
                        estado_pago=?
                        WHERE id=?""",
                     (data["cliente"], data["perfil"], data["telefono"],
                      data["plataforma_id"], data["orden_compra"],
                      data["correo_usuario"], data["proveedor_id"],
                      data["contrasena"], data["pin"], data["precio_venta"],
                      data["fecha_activacion"], data["fecha_vencimiento"],
                      data["notas"], data["estado_pago"], vid))
        conn.commit()
        conn.close()
    ...
    def get_resumen_ventas(self, periodo):
        ...
        if periodo == "dia":
            ...
            where = f"strftime('%Y-%m', v.fecha_registro) = strftime('%Y-%m', '{hoy}')"
        elif periodo == "anio":
            where = f"strftime('%Y', v.fecha_registro) = strftime('%Y', '{hoy}')"
        else:
            where = "1=1"

        total = conn.execute(
            f"SELECT COALESCE(SUM(precio_venta),0) as total, COUNT(*) as cantidad "
    ...
    def get_deudas(self):
        conn = self.get_conn()
        rows = conn.execute("""
            SELECT v.*, p.nombre as plataforma_nombre,
                   p.imagen_path as plataforma_imagen,
                   pr.nombre as proveedor_nombre
            FROM ventas v
            LEFT JOIN plataformas p ON v.plataforma_id = p.id
